...
export default async function CounselorCasesPage({ searchParams }: CasesPageProps) {
  ...
  return (
    <div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {statuses.map((st) => {
          const isActive = (!filterStatus && st === "ALL") || filterStatus === st;
          const href = st === "ALL" ? "/counselor/cases" : `/counselor/cases?status=${st}`;
          return (
            <Link
              key={st}
              href={href}
              ...
              cases.map((c) => (
                <tr key={c.id} className="hover:bg-secondary/30 transition">
                  <td className="px-4 py-3 font-mono font-semibold text-primary">
                    <Link href={`/counselor/cases/${c.id}`} className="hover:underline">
                      {c.case_ref}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-block rounded-full border px-2.5 py-0.5 text-xs font-medium ${
                        STATUS_BADGES[c.status] || "bg-secondary text-muted-foreground"
                      }`}
                    >
                      {c.status}
                    </span>
